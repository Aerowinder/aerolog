(function () {
  ...
  function activeRequestPriority(cause) {
    return cause === 'poll' ? 1 : 2;
  }

  function shouldSupersedeActiveRequest(nextCause) {
    const activeCause = App.state.runtime.request.cause;
    if (!activeCause) return true;
    return activeRequestPriority(nextCause) >= activeRequestPriority(activeCause) && nextCause !== 'poll';
  }

  function scheduleRequestTimeout(cause, requestId) {
    clearRequestTimeout();
    if (cause !== 'poll') return;
    const timeoutMs = App.derive.pollIntervalMs();
    if (!timeoutMs) return;
    App.state.runtime.request.timeoutId = setTimeout(() => {
      ...
      App.state.runtime.connection.detail = `Poll did not return within ${timeoutMs}ms`;
  ...
  async function dispatchRefresh(cause = 'manual') {
    ...
    const countPromise = runQuery(App.query.buildCountQuery(), signal);
    ...
    if (countError && isAbortError(countError) && logsResult.status !== 'fulfilled') {
      return { started: false, aborted: true };
    }
    ...
    App.state.runtime.totalPages = Math.max(1, Math.ceil(App.state.runtime.totalCount / parseInt(App.state.config.pageSize, 10)));
